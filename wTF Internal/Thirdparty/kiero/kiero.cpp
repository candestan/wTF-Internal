...
#ifdef _UNICODE
# define KIERO_TEXT(text) _(L##text)
#else
# define KIERO_TEXT(text) _(text)
#endif
...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			HWND window = ::CreateWindow(windowClass.lpszClassName, KIERO_TEXT("dxWNDW"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);
			...
			if (_renderType == RenderType::D3D9)
			{
				...
				if ((Direct3DCreate9 = ::GetProcAddress(libD3D9, KIERO_TEXT("Direct3DCreate9"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
